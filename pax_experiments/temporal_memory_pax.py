...
class TemporalMemory(object):
  ...
  def __init__(self,
    ...
    self.predictiveCells = np.array([])
    self.activeSegments = set()
    self.winnerCells = set()
    
    self.mCellRates = np.zeros(self.columnDimensions)
    self.prevPredRates = np.zeros(self.columnDimensions * self.cellsPerColumn)
    self.prevBurstCells = np.zeros(self.columnDimensions * self.cellsPerColumn)
    self.predictionW = 0.02 * np.random.rand(self.columnDimensions * self.cellsPerColumn, self.columnDimensions * self.cellsPerColumn)
    self.mCellW = np.ones(self.columnDimensions)   
    
    self.mCellLearningRate = 0.01
    self.mCellTargetSparsity = 0.2
  ...
  def computeFn(self,
    ...
    @param activeColumns                   (array)         Indices of active columns in `t`
    @param prevPredRates             (array)         Indices of predictive cells in `t-1`
  ...
  def step(self, activeColumns, learn=True):
    
    (predictedLevels, 
     activeCells, 
     burstCells) = self.activateCells(self.prevPredRates, self.mCellRates, activeColumns)
    
    
    if learn:
      self.predictionPlasticity(predictedLevels, burstCells, self.prevBurstCells)
    
    (prevPredRates, mCellRates) = self.computeNextPrediction(burstCells)
    
    self.prevPredRates = prevPredRates
    self.mCellRates = mCellRates
    self.prevBurstCells = burstCells
    
    self.activeCells = np.nonzero(activeCells)[1]
    self.predictiveCells = np.nonzero(burstCells)[1]
    
    return (activeCells, burstCells)
  ...
  def activateCells(self, prevPredRates, mCellRates, activeColumns):
    """
    ...
    # expand the "M" cell to get a rate for each cell
    mCellCellRates = np.reshape(np.tile(mCellRates, (self.cellsPerColumn, 1)).T, (1, -1))
    
    #predictedLevels = np.minimum((prevPredRates - mCellCellRates) + , 5)
    #predictedLevels = 20.0 * (1.0 / (1.0 + np.exp(-(prevPredRates - mCellCellRates))) - 0.3)
    predictedLevels = prevPredRates - mCellCellRates + 1
    
    activeCells = np.reshape(np.tile(activeColumns, (self.cellsPerColumn, 1)).T, (1, -1))

    burstCells = (predictedLevels > 0) * activeCells
    
    return predictedLevels, activeCells, burstCells
    
  def predictionPlasticity(self, predictedLevels, burstCells, prevBurstCells):
  
    pred_mat = np.tile(predictedLevels, (self.columnDimensions * self.cellsPerColumn, 1))    
    bc_mat = np.tile(burstCells, (self.columnDimensions * self.cellsPerColumn, 1))
    pbc_mat = np.tile(prevBurstCells, (self.columnDimensions * self.cellsPerColumn, 1)).T
    
    # Increment hebbian cells
    #self.predictionW += self.permanenceIncrement * pred_mat * bc_mat * pbc_mat
    self.predictionW += self.permanenceIncrement * pred_mat * bc_mat * pbc_mat
    
    self.predictionW[self.predictionW > 100] = 100
    
    # Decrement previous burst synapses
    self.predictionW -= self.permanenceDecrement * pbc_mat * (-bc_mat + 1) * self.predictionW # make it proportional decrement
    
    # Increase mCell gain with high burstiness
    burstCC = np.mean(np.reshape(burstCells, (self.columnDimensions, self.cellsPerColumn)), 1)
    
    self.mCellW += self.mCellLearningRate * (self.mCellTargetSparsity - burstCC)
    #self.mCellW -= self.mCellLearningRate * np.minimum(np.maximum(-1, np.log(burstCC/self.mCellTargetSparsity)), 1)
  
  def computeNextPrediction(self, burstCells):
    
    prevPredRates = np.dot(burstCells, self.predictionW)
    
    mCellRates = self.mCellW * np.mean(np.reshape(prevPredRates, (self.columnDimensions, self.cellsPerColumn)), 1)
    
    return (prevPredRates, mCellRates)
  ...
  def numberOfColumns(self):
    ...
    """
    return self.columnDimensions #reduce(mul, self.columnDimensions, 1)
