...
from union_pooling.activation.excite_functions.persistence_excite_function import (
  PersistenceExciteFunction)
...
class UnionPoolerNew(SpatialPooler):
  """
  ...
  def __init__(self,
    ...
    super(UnionPoolerNew, self).__init__(**kwargs)
    ...
    if exciteFunction is None:
      self._exciteFunction = PersistenceExciteFunction()
    else:
      self._exciteFunction = exciteFunction

    if decayFunction is None:
      self._decayFunction = ExponentialDecayFunction()
    else:
  ...
  def compute(self, activeInput, predictedActiveInput, learn):
    ...
    if self._fixedPoolingActivationBurst:
      ...
      # PoolingActivation update is based on overlap with predicted-active inputs
