...
def runSensorRegion(sensorRegion):
	retinaDimension = np.array(sensorRegion.encoder.getDimensions())
	retinaDimensionBeforeScaling = retinaDimension / sensorRegion.encoder.retinaScaling
	sensorInput = None
	sensorOutput = {'categoryOut': np.array([0]),
	                ...
	                'sequenceIdOut': [None],
	                'encodingOut': None,
	                'dataOut': np.zeros((retinaDimensionBeforeScaling[0] * retinaDimensionBeforeScaling[1],))}
	sensorRegion.compute(sensorInput, sensorOutput)
	sensorOutputSDR = np.zeros((retinaDimension[0] * retinaDimension[1],))
	sensorOutputSDR[sensorOutput['encodingOut']['fingerprint']['positions']] = 1

	return sensorOutput, sensorOutputSDR
