...
how to create a Apical Tiebreak TM instance, train it with vectors, get predictions, and
inspect the state.
...
for i in range(10):
  ...
  for j in range(5):
    activeColumns = [i for i, j in zip(count(), x[j]) if j == 1]

    # The compute method performs one step of learning and/or inference. Note:
    # here we just perform learning but you can perform prediction/inference and
    # learning in the same step if you want (online learning).

    tm.compute(activeColumns, apicalInput=(), apicalGrowthCandidates=None, learn=True)

    # The following print statements can be ignored.
    # Useful for tracing internal states

    print("active cells " + str(tm.getActiveCells()))
    print("predictive cells " + str(tm.getNextPredictedCells()))
...
for j in range(5):
  ...
  tm.compute(activeColumns, apicalInput=(), apicalGrowthCandidates=None, learn=False)
