...
esn = generate_esn(nInputUnits, nInternalUnits, nOutputUnits, ...
    'spectralRadius',.5,'inputScaling',.2*ones(nInputUnits,1),'inputShift',zeros(nInputUnits,1), ...
    ...
    'type', 'plain_esn', 'learningMode', 'online', 'RLS_delta', 0.1);
...
%     trainOutputSequence = outputSequence(i-nTrain:i-1, :);
    ...
    trainInputSequence = inputSequence(i, :);
    trainOutputSequence = outputSequence(i, :);

    %%%% train the ESN    
    [trainedEsn, stateMatrix] = train_esn(trainInputSequence, trainOutputSequence, esn) ;    
    
    % compute the output of the trained ESN on the training and testing data,
    % discarding the first nForgetPoints of each
    if i > nTrain
        numPtsSimulation = 100;
        predictedTrainOutput = test_esn(inputSequence(i-numPtsSimulation:i, :), ...
            trainedEsn, numPtsSimulation);
...
%%
startFrom = 6000;
sqrt(mean((predictedData(startFrom:end)-outputSequence(startFrom:end)).^2))/std(outputSequence(startFrom:end))

if injectPerturbation
    fileID = fopen('prediction/nyc_taxi_perturb_esn_online_pred.csv','w');
else
    fileID = fopen('prediction/nyc_taxi_esn_online_pred.csv','w');
end
