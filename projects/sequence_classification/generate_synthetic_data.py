...
def generateSequences(numSeq, numClass, sequenceLength, useMotif, sequenceMotifs):
  ...
  for seq in range(numSeq):
    ...
    motifStates.append(motifState)
  return trainData, motifStates
...
trainData, motifStates = generateSequences(numTrain, numClass, sequenceLength, useMotif, sequenceMotifs)
testData, motifStates = generateSequences(numTest, numClass, sequenceLength, useMotif, sequenceMotifs)
if not os.path.exists('SyntheticData/Test1'):
  os.makedirs('SyntheticData/Test1')
...
#plt.savefig('SyntheticData/Test1/Test1_TRAIN.png')
#plt.plot(motifState)

def plot_axis(axis, currentClass, sequence, motifState):
    axis.plot(sequence, 'k-')
    ...
    axis.set_xlim([0, 100])
    axis.set_ylabel('class {}'.format(currentClass))  
    

fig, ax = plt.subplots(nrows=4, ncols=1, figsize=(20,12))
idx = [1,2,numTrain/2,numTrain/2+1]
for i in range(4):
  currentClass = trainLabel[idx[i]]
  sequence = trainData[idx[i]]    
  motifState = motifStates[idx[i]]
  axis = ax[i]
  plot_axis(axis, currentClass, sequence, motifState)
