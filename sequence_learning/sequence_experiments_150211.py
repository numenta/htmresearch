...
150206: going to add in noise and see how it does.
150211: looking at true HO sequences
...
#%% Create a bunch of sequences which require high-order to look back

num_sequences = 8
sequence_order = 4

sdr_list = arange(max(100, num_sequences))

num_sdr_per_seq = sequence_order + 2

# The middle of all sequences is the same sequence
ho_sequence = randint(len(sdr_list), size=sequence_order)

sequence_set = zeros((num_sequences, num_sdr_per_seq))

sequence_set[:,0] = arange(num_sequences) + 1
sequence_set[:,-1] = arange(num_sequences) + 1

sequence_set[:,1:-1] = tile(ho_sequence, (num_sequences,1))
...
for i in range(n_sequence_displays):
  ...
  for j in range(num_sdr_per_seq):
    sdr_name = sequence_set[which_seq,j]
...
for i in [2]: #range(num_sequences):
  ...
  for j, trial in enumerate(display_trials):
    for k in range(num_sdr_per_seq):
      ...
      if k < (num_sdr_per_seq - 1):
        xticks([])
      else:
        print str(sequence_idxs[trial])
        xticks([1024], (str(sequence_idxs[trial]), ))
