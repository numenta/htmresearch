...
#TM_SM is a child class of TM
class TM_SM(TM):

  """
  Class implementing the sensorimotor inference on top of the basic sequence learning
  TM_SM is a child class of TM, which implements pure sequence learning
  """
  ...
  def __init__(self,
    ...
    @param numberOfDistalInput length of distal input
    ...
    @param learnLateralConnections Bool variable controlling whether lateral connections
                  between cells in the same region are used for prediction. If it is true
                  distal dendrites can receive inputs from lateral connections

    @param learnDistalInputs Bool variable controlling whether external inputs can be
                  used for prediction. If it is true, distal dendrites can receive
                  external input

    @param learnOnOneCell when set to True, learning will occur for one cell per column
                  until reset. This is useful in sensorimotor inference to encourage use
                  of different cells per column to represent different pattern context, rather
                  than sequence contexts
    ...
    self.newDistalSynapseCount = newDistalSynapseCount
    ...
    self.maxAge = maxAge
    self.learnLateralConnections = learnLateralConnections
    self.learnDistalInputs = learnDistalInputs
    ...
    # Keep integers rather than bools.
    self.activeState = {}
    ...
    self.learnState["seq"] = numpy.zeros(stateShape, dtype="int8")
    ...
    self.activationHistory = numpy.zeros(stateShape, dtype="int8")
    ...
    self.distalDendriticInput = {}
    self.distalDendriticInput["t"] = numpy.zeros((self.numberOfDistalInput,1), dtype="int8")
    self.distalDendriticInput["t-1"] = numpy.zeros((self.numberOfDistalInput,1), dtype="int8")
    ...
    self.iterationIdxreset = 0 # iteration index at most recent reset
  ...
  def reset(self,):
    ...
    self.learnState['seq'].fill(0)

    self.confidence['t-1'].fill(0)
    self.confidence['t'].fill(0)
    self.distalDendriticInput['t-1'].fill(0)
    self.distalDendriticInput['t'].fill(0)
    ...
    # record when the most recent reset
    self.iterationIdxreset = self.iterationIdx
  ...
  def printCell(self, c, i, onlyActiveSegments=False):
    ...
    if len(self.cells[c][i]) > 0:
      ...
      for j,s in enumerate(self.cells[c][i]):
        isActive = self.isSegmentActive(s, self.activeState['t'], self.distalDendriticInput['t'])
  ...
  def printStates(self, printPrevious = True, printLearnState = True):
    ...
    if printLearnState:
      ...
      print "Learn Seq state"
      for i in xrange(self.cellsPerColumn):
        print formatRow(self.learnState['seq'],i)
  ...
  #############################################################################
  def getNumDistalSynapses(self):
    """ Return the total number of synapses
    """
    nDSyns = self.getSegmentInfo()[2]
    return nDSyns
  ...
  def getSeqLearnCell(self, c):
    """ return learned cells that are chosen for this sequence"""

    # check whether any cells are assigned for this sequence
    learnCell = numpy.where(self.learnState['seq'][c] == 1)[0]
    if len(learnCell) == 0:
      # no cells have been assigned yet, randomly pick one as learn cell
      return []
    else:
      learnCell = learnCell[0]
  ...
  class SegmentUpdate():
    ...
    def __init__(self, c, i, seg=None, activeSynapses=[], activeDistalSynapses=[]):
      """
          ...
          @param activeSynapses: active lateral synapses

          @param activeDistalSynapses: active distant synapses that receives external input
      """
      ...
      self.activeDistalSynapses = activeDistalSynapses # active distal synapses
    ...
    def __str__(self):
      ...
      return "Seg update: cell=[%d,%d]" % (self.columnIdx, self.cellIdx)  \
             ...
             + ", distal synapses=" + str(self.activeDistalSynapses) \
  ...
  def addToSegmentUpdates(self, c, i, segUpdate):
    ...
    if segUpdate is None or (len(segUpdate.activeSynapses) == 0 \
      and len(segUpdate.activeDistalSynapses)==0):
      return
  ...
  def computePhase2(self, doLearn=False):
    ...
    this phase, all the cell with distal support have their predictedState
    ...
    # - if a segment has enough activity, either due to horizontal or distal
    # dendritic input, it's set to be predicting, and we queue up the segment
    #   for reinforcement,
    for c in xrange(self.numberOfCols):
      ...
      for i in xrange(self.cellsPerColumn):
        ...
        for s in self.cells[c][i]:
          ...
          if self.isSegmentActive(s, self.activeState['t'],
            ...
            # if doLearn:
            #   # penalize false alarm
            #   if self.predictedState['t-1'][c][i]==1 and self.activeState['t'][c][i]==0:
            #     activeUpdate = self.getSegmentActiveSynapses(c, i, s, 't-1')
            #     activeUpdate.phase1Flag = False
            #     self.addToSegmentUpdates(c, i, activeUpdate)
  ...
  def compute(self, bottomUpInput, distalDendriticInput, enableLearn, computeInfOutput=None):
    ...
    if enableLearn:
      self.learn(bottomUpInput, distalDendriticInput)
    else:
      self.infer(bottomUpInput, distalDendriticInput)
  ...
  #
  ################################################################################
  def infer(self, bottomUpInput, distalDendriticInput):
    """
    Parameters:
    --------------------------------------------
    ...
    self.distalDendriticInput['t-1'][:,:] = self.distalDendriticInput['t'][:,:]

    self.activeState['t'].fill(0)
    self.predictedState['t'].fill(0)
    self.confidence['t'].fill(0.0)
    self.learnState['t'][:,:] = 0
    self.distalDendriticInput['t'][:,0] = distalDendriticInput
    ...
    for c in activeColumns:
      ...
      # not predicted
      # Will not engage learning if reset has just been called, since it takes
      # one step to make prediction
      if not buPredicted :
      # if not buPredicted:
  ...
  ################################################################################
  def learn(self, bottomUpInput, distalDendriticInput):
    """
    Parameters:
    --------------------------------------------
    ...
    for c in activeColumns:
      ...
      # Will not engage learning if reset has just been called, since it takes
      # one step to make prediction
      # if not buPredicted and (self.iterationIdx - self.iterationIdxreset) > 1:
      ...
      if not buPredicted :
        ...
        # We didn't find a cell to learn on (since no cell was predicted), so
        # just pick the best matching cell that has,
        # sum(all synapses) >= minThreshold, "weak" activation
        # or, failing that, the least allocated
        # cell.
  ...
  ################################################################################
  def trimSegmentsInCell(self, colIdx, cellIdx, segList, minPermanence, minNumSyns):
    ...
    nSegsRemoved, nSynsRemoved, ndSynsRemoved = 0, 0, 0
    ...
    for segment in segList:
      ...
      dsynsToDel = [syn for syn in segment.dsyns if syn[2] < minPermanence]

      if (len(synsToDel) == len(segment.syns) ) and (len(dsynsToDel)==len(segment.dsyns)):
        segsToDel.append(segment) # will remove the whole segment
      else:
        if len(synsToDel) > 0:
          for syn in synsToDel: # remove some synapses on segment
            ...
            nSynsRemoved += 1
        if len(dsynsToDel) > 0:
          for syn in dsynsToDel: # remove some synapses on segment
            segment.dsyns.remove(syn)
            ndSynsRemoved += 1
        if len(segment.syns)+len(segment.dsyns) < minNumSyns:
    ...
    return nSegsRemoved, nSynsRemoved, ndSynsRemoved
  ...
  def trimSegments(self, minPermanence=None, minNumSyns=None):
    ...
    totalSegsRemoved, totalSynsRemoved, totaldSynsRemoved = 0, 0
    for c,i in product(xrange(self.numberOfCols), xrange(self.cellsPerColumn)):

      (segsRemoved, synsRemoved, dsynsRemoved) = self.trimSegmentsInCell(colIdx=c, cellIdx=i,
      ...
      totaldSynsRemoved += dsynsRemoved
  ...
  #############################################################################
  def getSegmentActivityLevel(self, seg, activeState, distalDendriticInput,
    ...
    # Computing in C++, use function getSegmentActivityLevel
    lateralActivity = getSegmentActivityLevel(seg.syns, activeState,
                                              connectedSynapsesOnly,
                                              self.connectedPerm)

    distalActivity = getSegmentActivityLevel(seg.dsyns, distalDendriticInput,
                                             connectedSynapsesOnly,
                                             self.connectedPerm)

    return lateralActivity+distalActivity
  ...
  def getSegmentActiveSynapses(self, c,i,s, timeStep, newSynapses =False):
    ...
    distalDendriticInput = self.distalDendriticInput[timeStep]

    if s is not None: # s can be None, if adding a new segment
      if self.learnLateralConnections:
        # Here we add *integers* to activeSynapses
        activeSynapses = [idx for idx, syn in enumerate(s.syns) \
                          if activeState[syn[0], syn[1]]]

      if self.learnDistalInputs:
        # Here we add active distal dendritic that receives external input
        activeDistalSynapses = [idx for idx, syn in enumerate(s.dsyns) \
                          if distalDendriticInput[syn[0], 0]]
    ...
    if newSynapses: # add a few more synapses
      ...
      if self.learnDistalInputs:
        ...
        nDistalSynapsesToAdd = self.newDistalSynapseCount - len(activeDistalSynapses)

        # Here we add *pairs* (colIdx, cellIdx) to activeSynapses
        activeDistalSynapses += self.chooseDistalDendriticInputToLearnFrom(c,i,s,\
                               nDistalSynapsesToAdd, timeStep)
    ...
    #   segment
    update = TM_SM.SegmentUpdate(c, i, s, activeSynapses, activeDistalSynapses)
    return update



  #############################################################################
  def isSegmentActive(self, seg, activeState, distalDendriticInput):
    """
    ...
    return (self.getSegmentActivityLevel(seg, activeState,
      distalDendriticInput, connectedSynapsesOnly=True) > self.activationThreshold)

    # todo: adapt the C++ code to handle dsyn
    # return isSegmentActive(seg.syns, activeState,
    #                        self.connectedPerm, self.activationThreshold)

  ##############################################################################
  def chooseDistalDendriticInputToLearnFrom(self, c,i,s, n, timeStep):
    ...
    if timeStep == 't-1':
      tmpCandidates = numpy.where(self.distalDendriticInput['t-1'] == 1)
    else:
      tmpCandidates = numpy.where(self.distalDendriticInput['t'] == 1)
    ...
    if s is None: # new segment
      ...
      synapsesAlreadyInSegment = set((syn[0], syn[1]) for syn in s.dsyns)
      cands = [syn for syn in zip(tmpCandidates[0], tmpCandidates[1]) \
  ...
  def getBestMatchingCell(self, c, activeState, distalDendriticInput):
    """Find weakly activated cell in column.
    Activity of Cell = maximal activity of all segment
    Consider all synapses (connected and unconnected) when calculate activity
    If activity of most active cell >= minThreshold:
      Returns index of the most active cell and most activated segment
    Otherwise
      Return (None, None)
    """
    ...
    for i in xrange(self.cellsPerColumn):
      ...
      # find most active segment of cell i in column c
      maxSegActivity = 0
      maxSegIdx = 0

      for j,s in enumerate(self.cells[c][i]):

        # todo: add lateral inputs here?
        activity = self.getSegmentActivityLevel(s, activeState,
                  distalDendriticInput, connectedSynapsesOnly=False)

        if self.verbosity >= 6:
          print " Segment Activity for column ", c, " cell ", i, \
                " segment ", j, " is ", activity

        if activity > maxSegActivity:
          maxSegActivity = activity
          maxSegIdx = j

      # find most active cell in the column (cell with most active segment)
    ...
    if bestCellInCol == -1:
      # if no cell is above minThresh
      return (None, None)
    else:
      # return most active cell and most active segments
      return bestCellInCol, self.cells[c][bestCellInCol][bestSegIdxInCol]


  def getBestMatchingSegment(self, c, i, activeState, distalDendriticInput):
    ...
    above minThreshold. The routine returns the segment object. If no
    segments are found, then None is returned.
    """
    maxActivity = self.minThreshold
    bestSegment = None
    ...
    for key, updateList in self.segmentUpdates.iteritems():
      ...
      for (createDate, segUpdate) in updateList:
        ...
        if self.iterationIdx - createDate > self.segUpdateValidDuration:
          ...
          pass
        elif (positiveReinforcement is not None):
  ...
  def adaptSegment(self, segUpdate, positiveReinforcement):
    ...
    activeLateralSynapses = segUpdate.activeSynapses
    activeDistalSynapses = segUpdate.activeDistalSynapses

    lateralSynToUpdate = set([syn for syn in activeLateralSynapses if type(syn) == int])
    distalSynToUpdate = set([syn for syn in activeDistalSynapses if type(syn) == int])
    ...
    if segment is not None: # modify an existing segment
      ...
      if positiveReinforcement:
        ...
        lastLateralSynIndex = len(segment.syns) - 1
        inactiveLateralSynIndices = [s for s in xrange(0, lastLateralSynIndex+1) \
                              if s not in lateralSynToUpdate]
        lastDistalSynIndex = len(segment.dsyns) - 1
        inactiveDistalSynIndices = [s for s in xrange(0, lastDistalSynIndex+1) \
                              if s not in distalSynToUpdate]
        trimSegment = segment.updateSynapses(inactiveLateralSynIndices,
                             inactiveDistalSynIndices, -self.permanenceDec)


        # Now, increment active synapses
        activeLateralSynIndices = [syn for syn in lateralSynToUpdate if syn <= lastLateralSynIndex]
        activeDistalSynIndices = [syn for syn in distalSynToUpdate if syn <= lastDistalSynIndex]

        segment.updateSynapses(activeLateralSynIndices, activeDistalSynIndices,
                self.permanenceInc)

        # Finally, create new synapses if needed
        # syn is now a tuple (src col, src cell)
        LateralSynsToAdd = [syn for syn in activeLateralSynapses if type(syn) != int]
        DistalSynsToAdd = [syn for syn in activeDistalSynapses if type(syn) != int]

        for newSyn in LateralSynsToAdd:
          segment.addSynapse(newSyn[0], newSyn[1], self.initialPerm)

        for newSyn in DistalSynsToAdd:
          segment.addDistalSynapse(newSyn[0], 0, self.initialPerm)
        ...
        trimSegment = segment.updateSynapses(lateralSynToUpdate,
                      distalSynToUpdate, -self.permanenceDec)
      ...
      for synapse in activeLateralSynapses:
        newSegment.addSynapse(synapse[0], synapse[1], self.initialPerm)

      for synapse in activeDistalSynapses:
        newSegment.addDistalSynapse(synapse[0], 0, self.initialPerm)
  ...
  def getSegmentInfo(self, collectActiveData = False):
    ...
    (
      ...
      nDistSynapses,    # total number of distal synapses
      ...
      distDSegSizes,    # a dict where d[n] = number of segments with n distal synapses
    ...
    nSegments, nSynapses, nDistSynapses = 0, 0, 0
    nActiveSegs, nActiveSynapses = 0, 0
    distSegSizes, distDSegSizes, distNSegsPerCell = {},{}, {}
    ...
    numSegmentPerCell = numpy.zeros((self.numberOfCols, self.cellsPerColumn))
    ...
    for c in xrange(self.numberOfCols):
      for i in xrange(self.cellsPerColumn):
        ...
        if len(self.cells[c][i]) > 0:
          ...
          numSegmentPerCell[c][i] = nSegmentsThisCell
          ...
          for seg in self.cells[c][i]:
            ...
            nDistSynapsesThisSeg = seg.getNumDistalSynapses()
            nSynapses += nSynapsesThisSeg
            nDistSynapses += nDistSynapsesThisSeg
            ...
            if distDSegSizes.has_key(nDistSynapsesThisSeg):
              distDSegSizes[nDistSynapsesThisSeg] += 1
            else:
              distDSegSizes[nDistSynapsesThisSeg] = 1

            # print " (",c,",",i,")","nSegment=",nSegmentsThisCell,"seg",seg,\
            #       "nSynapse=",nSynapsesThisSeg,"nDSynapse=",nDistSynapsesThisSeg
            ...
            if collectActiveData:
              if self.isSegmentActive(seg, self.ActiveState['t'],self.distalDendriticInput['t']):
                nActiveSegs += 1
              for syn in seg.syns:
                if self.activeState['t'][syn[0]][syn[1]] == 1:
                  nActiveSynapses += 1

    return (nSegments, nSynapses, nDistSynapses, nActiveSegs, nActiveSynapses,
            distSegSizes, distNSegsPerCell, distPermValues, distAges, numSegmentPerCell)
...
class Segment(object):
  ...
  def __init__(self, tp, isSequenceSeg):
    ...
    # synapses that receive lateral connections
    self.syns = []

    # Distant synapses that receive external input
    self.dsyns = []
  ...
  def printSegment(self):
    ...
    print
    print "     Distal Dendrite that receives external inputs: ",
    sortedSyns = sorted(self.dsyns)
    for _, synapse in enumerate(sortedSyns):
      print "[%d,%d]%4.2f" % (synapse[0], synapse[1], synapse[2]),
    print
  ...
  def addSynapse(self, srcCellCol, srcCellIdx, perm):
    """Add a new synapse that receive lateral connections
  ...
  def addDistalSynapse(self, srcCellCol, srcCellIdx, perm):
    """Add a new synapse that connects to distal inputs

    @param srcCellCol source cell column
    @param srcCellIdx source cell index within the column
    @param perm       initial permanence
    """
    self.dsyns.append([int(srcCellCol), int(srcCellIdx), numpy.float32(perm)])

  def updateSynapses(self, lateralSynapses, distalSynapses, delta):
    """Update a set of synapses in the segment.

    @param tp       The owner TP
    @param lateralSynapses List of lateral synapse indices to update
    @param distalSynapse List of distal synapse indices to update
    ...
    if delta > 0:
      for synapse in lateralSynapses:
      ...
      for synapse in distalSynapses:
        self.dsyns[synapse][2] = newValue = self.dsyns[synapse][2] + delta

        # Cap synapse permanence at permanenceMax
        if newValue > self.tp.permanenceMax:
          self.dsyns[synapse][2] = self.tp.permanenceMax
