...
from htmresearch.algorithms.subsampling_column_pooler import SubsamplingColumnPooler
...
def getConstructorArguments():
  ...
  """
  argspec = inspect.getargspec(SubsamplingColumnPooler.__init__)
  return argspec.args[1:], argspec.defaults


class SubsamplingColumnPoolerRegion(PyRegion):
  """
  The SubsamplingColumnPoolerRegion implements an L2 layer within a single cortical column / cortical
  module.
    ...
    spec = dict(
      ...
      outputs=dict(
        feedForwardOutput=dict(
          description="The default output of SubsamplingColumnPoolerRegion. By default this"
      ...
      parameters=dict(
        ...
        # Proximal
        #
        lateralSubsamplingFactors=dict(
          description="Sizes of subsamples for lateral inputs",
          accessMode="Read",
          dataType="Real32",
          count=0),
        lateralVoteWeights=dict(
          description="Weights given to lateral inputs in voting",
          accessMode="Read",
          dataType="Real32",
          count=0),
  ...
  def __init__(self,
               ...
               lateralSubsamplingFactors = None,
               lateralVoteWeights = None,
               lateralInputWidths = None,
    ...
    self.lateralSubsampleSizes = [int(width*factor) for width, factor in zip(
        self.lateralInputWidths, self.lateralSubsamplingFactors)]
  ...
  def initialize(self):
    ...
    if self._pooler is None:
      params = {
        ...
        "lateralSubsampleSizes": self.lateralSubsampleSizes,
        "lateralSubsamplingFactors" : self.lateralSubsamplingFactors,
        "lateralVoteWeights": self.lateralVoteWeights,
