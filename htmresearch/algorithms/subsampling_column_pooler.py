...
class SubsamplingColumnPooler(object):
  """
  ...
  def __init__(self,
               inputWidth,
               lateralInputWidths=(),
               lateralSubsampleSizes = (),
               lateralSubsamplingFactors = (),
               lateralVoteWeights = (),
               cellCount=4096,
               sdrSize=40,
    ...
    @param  lateralSubsampleSizes (list of ints)
            The number of bits to consider from each lateral input

    @param  lateralVoteWeights (list of ints)
            The weight that each lateral input should be assigned.
            Typically, distant columns are given less voting power, as normally
            less input is received from them.
    ...
    self.potentialPools = tuple(numpy.random.choice(inputWidth, subsampleSize,
        replace = False) for inputWidth, subsampleSize in
        zip(lateralInputWidths, lateralSubsampleSizes))
  ...
  def _computeInferenceMode(self, feedforwardInput, lateralInputs):
    ...
    for i, (lateralInput, votePower) in enumerate(zip(lateralInputs,
        self.lateralVoteWeights)):
      overlaps = self.distalPermanences[i].rightVecSumAtNZGteThresholdSparse(
        lateralInput, self.connectedPermanenceDistal)
      numActiveSegmentsByCell[overlaps >=
          self.activationThresholdDistal] += votePower
