...
from fluent.encoders.vec_encoder import VecEncoder
...
class ClassificationModelVector(ClassificationModel):
  """
  Class to run the survey response classification task with encoded 
  word vectors.
  ...
  def __init__(self,
    ...
    self.encoder = VecEncoder(vectorsPath=vectorsPath)
    self.n = self.encoder.n
    self.w = self.encoder.w
